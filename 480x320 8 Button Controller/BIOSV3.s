...
;8 push buttons: on port A A2-A0 (A0-data, A1 - shift, A2 - Clock) also wired to 4 interupts
;
...
CONTROLLER_STATUS   = $0215       ;Status of all 8 controller buttons bit 0 - A, bit 1 - B, bit 2 - C, bit 3 - D, bit 4 - W, bit 5 - X, bit 6 - Y, bit 7 - Z
...
reset:
  ...
  sta DDRB
  lda #%11110110                            ;TFT control pins A4-A7, I/O for controller A0-A2, A3 is unused.
  sta DDRA
  lda #%11110010                            ;set all control pins to high to start (note RESET is hard wired high!), controller outputs start low
  sta PORTA
...
IRQ:
  ...
  beq irq_button_A_jmp                      ;button A pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%01000000                            ;see if it was button B
  beq irq_button_B_jmp                      ;button B pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00100000                            ;see if it was button C
  beq irq_button_C_jmp                      ;button C pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00010000                            ;see if it was button D
  beq irq_button_D_jmp                      ;button D pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00001000                            ;see if it was button W
  beq irq_button_W_jmp                      ;button W pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00000100                            ;see if it was button X
  beq irq_button_X_jmp                      ;button X pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00000010                            ;see if it was button Y
  beq irq_button_Y_jmp                      ;button Y pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00000001                            ;see if it was button Z
  beq irq_button_Z_jmp                      ;button Z pressed
