...
;Version 5
;
;Same game but has compatibility with the 8 button controller
;Uses BIOS Version 3
;
;Version 4
;
;Same game but initialise a much bigger screen - 3.5" 480 x 320
;Uses BIOS Version 2 - 4 button controller
;
;initialise screen - done
;fast fill screen - fixed longstanding bug with screen resolution, it is a wonder it ever worked!
;parameters to write characters to any point on screen - done but there is a bug on row 4, hacked location coordinates to plaster over problem
;Fix delay to work with much faster processor (BIOS CHANGE) - delay_ms - done for 8MHz
;
;Version 3
;
;Playing with altering code to work with the new screen - 3.5" 480 x 320 - remaining specific setup for this resolution - see V4
;Screen variables - done
;New screen clear routines (BIOS CHANGE) - done
;
;Version 2 
...
CLEMO_CLOSENESS         = $08     ;How close we can get to Clemo before she starts to run away!
...
TFT_HALF_COUNTER_H  = $0304       ;counter for faster TFT drawing counts half the screen for H
TFT_HALF_COUNTER_W  = $0305       ;counter for faster TFT drawing counts half the screen for W
TFT_PIXEL_COLOUR    = $0306       ;location of offset of current pixel colour foreground (an increment from black)
TFT_PIXEL_COLOUR_BG = $0307       ;location of offset of current pixel colour background (increment from black)
TFT_CURSOR_H        = $0308       ;location of cursor in y
TFT_CURSOR_H_MSB    = $0309       ;location of cursor in y - MSB
TFT_CURSOR_H_MSB_END= $030a       ;location of cursor end in y - required for row 4
TFT_CURSOR_H_END    = $030b       ;location of cursor end in y (top right of character)
TFT_CURSOR_H_ROW    = $030c       ;location of cursor in number of rows
TFT_CURSOR_W        = $030d       ;location of cursor in x
TFT_CURSOR_W_MSB    = $030e       ;location of cursor in x - MSB
TFT_CURSOR_W_MSB_END= $030f       ;location of cursor end in x - MSB - required for column 16
TFT_CURSOR_W_END    = $0310       ;location of cursor end in x (top right of character)
TFT_CURSOR_W_COL    = $0311       ;location of cursor in number of columns
TFT_BYTE            = $0312       ;current byte from character map
TFT_BYTE_OFFSET     = $0313       ;offset to current character from start of character map - (x by 16 to get actual offset)

STRING_LENGTH       = $0314       ;length of a string for printing

CONTROLLER_STATUS   = $0315       ;Status of all 8 controller buttons bit 0 - A, bit 1 - B, bit 2 - C, bit 3 - D, bit 4 - W, bit 5 - X, bit 6 - Y, bit 7 - Z
...
reset:
  ...
  sta DDRB
  lda #%11110110                            ;Set D0-D3 on port A for output - TFT control (note removed one line to add in additional button)
  sta DDRA
  lda #%11110010                            ;set all control pins to high to start (note RESET is hard wired high!)
  sta PORTA
...
Get_Tile_Number:;function to get a tile number from H and W coordinates
  ...
  lsr TEMP_1                                 ;divide by 2 to account to translate from char coordinates to level coordinates
...
;there is a bug somewhere in the AI which causes Clemo to sometimes do whatever she wants - I've left it in as its actually quite fun!
...
spawn_draw_clemo:
 ...
 check_buttons:
  ...
  jsr Read_Controller                       ;get the controller status in a CONTROLLER_STATUS
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%10000000                            ;see if it was button A
  beq irq_button_A                          ;button A pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%01000000                            ;see if it was button B
  beq irq_button_B                          ;button B pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00100000                            ;see if it was button C
  beq irq_button_C                          ;button C pressed
  lda CONTROLLER_STATUS                     ;load into a register
  cmp #%00010000                            ;see if it was button D
  beq irq_button_D                          ;button D pressed
  ;W,X,Y,Z not checked as they aren't used in Catch Clemo!
...
TFT_Write_Com:;Function to write a command to screen bus
  ...
  lda #%11000010                      ;set RS low (TFT_CS & TFT_RS)
  sta PORTA             
  pla                                 ;pull char back off stack

  sta PORTB                           ;output char onto bus
  lda #%10000010                      ;set tft_wr low (TFT_CS & TFT_RS & TFT_WR)
  sta PORTA
  lda #%11000010                      ;put back into previous state (TFT_CS & TFT_RS)
  sta PORTA
...
TFT_Write_Data:;Function to write data to screen bus
  ...
  pha                         ;put char onto stack
  lda #%11100010              ;ensure RS High (TFT_CS)
  sta PORTA             
  pla                         ;pull char back off stack
...
Read_Controller:;Function to read the controller input into the A register 0 - not pressed, 1 - pressed
  
  lda #$00                          ;clear the CONTROLLER_STATUS
  sta CONTROLLER_STATUS
  
  lda #%11100000                    ;Pulse shift low to get button state, Note keep TFT in current state
  sta PORTA
  lda #%11100010                    ;Pulse shift high to get button state, Note keep TFT in current state
  sta PORTA

  ;get button inputs 
  lda #$08                          ;set up COUNTER_I to count 8 buttons
  sta COUNTER_I
  
read_button:  
  clc                               ;clear carry so it doesn't get sucked into controller status when rotating
  rol CONTROLLER_STATUS             ;clear bit 0 of CONTROLLER_STATUS but save previously read bits
  lda PORTA                         ;Read current state of all of port A
  and #%00000001                    ;isolate button press
  adc CONTROLLER_STATUS             ;add in existing value of CONTROLLER_STATUS
  sta CONTROLLER_STATUS             ;store the result

  lda #%11100110                    ;Pulse clock, Note keep TFT in current state
  sta PORTA
  lda #%11100010                    ;Pulse clock, Note keep TFT in current state
  sta PORTA

  dec COUNTER_I 
  bne read_button                   ;still have buttons left to read so read the next one
